...
if [ -f $VEN/etc/audio_output_policy.conf ] && [ -f $SYS/etc/audio_policy_configuration.xml ]; then
  ...
  if $MAGISK; then
    ...
    [ ! -f $ORIGDIR$VEN/etc/audio/audio_policy_configuration.xml.bak ] && cp_ch $ORIGDIR$VEN/etc/audio/audio_policy_configuration.xml $UNITY$VEN/etc/audio/audio_policy_configuration.xml.bak
  fi
