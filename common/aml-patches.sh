...
if [ -f $V_AUD_OUT_POL ] && [ -f $AUD_POL_CONF ]; then
  sed -i '/Speaker/{n;s/deep_buffer,/`#deep_buffer,`/;}' $AMLPATH$AUD_POL_CONF
  sed -i '/Wired Headset/{n;s/deep_buffer,/`#deep_buffer,`/;}' $AMLPATH$AUD_POL_CONF
  sed -i '/Wired Headphones/{n;s/deep_buffer,/`#deep_buffer,`/;}' $AMLPATH$AUD_POL_CONF
elif [ ! -f $V_AUD_OUT_POL ] && [ -f $AUD_POL_CONF ]; then
  sed -i 's/deep_buffer,/`#deep_buffer,`/g' $AMLPATH$AUD_POL_CONF
  sed -i 's/,deep_buffer/`#,deep_buffer`/g' $AMLPATH$AUD_POL_CONF
else
