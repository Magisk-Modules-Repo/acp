...
dump_boot() {
  ...
  if [ -f "$bin/nanddump" ]; then
    $bin/nanddump -f $AK2/boot.img $block;
  else
    dd if=$block of=$AK2/boot.img;
  fi;
  if [ "$(strings $AK2/boot.img | grep -E 'Green Loader|Green Recovery')" ]; then
    mv -f $AK2/boot.img $AK2/boot-orig.img;
    dd bs=1048576 count=1 conv=notrunc if=$AK2/boot-orig.img of=$split_img/boot.img-master_boot.key;
    dd bs=1048576 skip=1 conv=notrunc if=$AK2/boot-orig.img of=$AK2/boot.img;
  fi;
  if [ -f "$bin/unpackelf" -a "$($bin/unpackelf -i $AK2/boot.img -h -q 2>/dev/null; echo $?)" == 0 ]; then
    $bin/unpackelf -i $AK2/boot.img -o $split_img;
    mv -f $split_img/boot.img-ramdisk.cpio.gz $split_img/boot.img-ramdisk.gz;
  elif [ -f "$bin/dumpimage" ]; then
    $bin/dumpimage -l $AK2/boot.img;
    $bin/dumpimage -l $AK2/boot.img > $split_img/boot.img-header;
    ...
    $bin/dumpimage -i $AK2/boot.img -p 0 $split_img/boot.img-zImage;
    test $? != 0 && dumpfail=1;
    if [ "$(cat $split_img/boot.img-type)" == "Multi" ]; then
      $bin/dumpimage -i $AK2/boot.img -p 1 $split_img/boot.img-ramdisk.gz;
    else
      dumpfail=1;
    fi;
  elif [ -f "$bin/rkcrc" ]; then
    dd bs=4096 skip=8 iflag=skip_bytes conv=notrunc if=$AK2/boot.img of=$split_img/boot.img-ramdisk.gz;
  elif [ -f "$bin/pxa-unpackbootimg" ]; then
    $bin/pxa-unpackbootimg -i $AK2/boot.img -o $split_img;
  else
    $bin/unpackbootimg -i $AK2/boot.img -o $split_img;
  fi;
...
write_boot() {
  ...
  if [ -f "$bin/mkbootfs" ]; then
    $bin/mkbootfs $ramdisk | gzip > $AK2/ramdisk-new.cpio.gz;
  else
    cd $ramdisk;
    find . | cpio -H newc -o | gzip > $AK2/ramdisk-new.cpio.gz;
  fi;
  if [ $? != 0 ]; then
  ...
  if [ -f "/data/custom_boot_image_patch.sh" ]; then
    ash /data/custom_boot_image_patch.sh $AK2/boot-new.img;
    if [ $? != 0 ]; then
      ui_print " "; ui_print "User script execution failed. Aborting..."; exit 1;
    fi;
  fi;
  if [ "$(strings $AK2/boot.img | grep SEANDROIDENFORCE )" ]; then
  ...
  if [ -f "$bin/flash_erase" -a -f "$bin/nandwrite" ]; then
    ...
    $bin/nandwrite -p $block $AK2/boot-new.img;
  else
    dd if=/dev/zero of=$block 2>/dev/null;
    dd if=$AK2/boot-new.img of=$block;
  fi;
}
